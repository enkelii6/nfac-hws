...
def recursive_reverse(my_list: list) -> list:
    return [] if not my_list else [my_list[-1]] + recursive_reverse(my_list[:-1])
...
def count_calls(func):
    ...
    def wrapper(*args, **kwargs):
        nonlocal calls
        calls += 1
        print(f"'{func.__name__}' was called {calls} times.")
...
def recursive_sum(my_list: list) -> int:
    return 0 if not my_list else my_list[0] + recursive_sum(my_list[1:])
